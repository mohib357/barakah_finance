...
function applyTheme(t) {
  ...
  // Sync both storage keys for cross-page consistency
  localStorage.setItem('bf_theme', t);
  localStorage.setItem('bf_dark', t === 'dark' ? '1' : '0');
  const icon = document.getElementById('themeIcon');
  if (icon) icon.textContent = t === 'dark' ? '☀️' : '🌙';
  // Sync index.html dk-tog if present
  const dkTog = document.getElementById('dkTog');
  if (dkTog) dkTog.classList.toggle('on', t === 'dark');
}

// toggleDarkMode is the unified function (used by index.html dk-tog)
function toggleDarkMode() {
  const current = localStorage.getItem('bf_dark') === '1' ? 'dark' : 'light';
